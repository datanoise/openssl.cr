...
class OpenSSL::HMAC
  ...
  def self.new(key, digest)
    new.tap do |hmac|
      LibCrypto.hmac_init_ex(hmac, key.cstr as Pointer(Void), key.bytesize, digest.to_unsafe_md, nil)
    end
  end
  ...
  def update(data)
    LibCrypto.hmac_update(self, data, LibC::SizeT.cast(data.bytesize))
    self
  end
